...
void PCA9685_Init(PCA9685 *pca, I2C_HandleTypeDef *I2Chandle) {
	...
	pca->preScale = (uint8_t) ((PCA9685_OSC_FREQ / (pwmFreq * 4096.0f) + 0.5f) - 1.0f); //p.25

	/* Reset */
	uint8_t txBuf[] = {PCA9685_REG_MODE1, 0x80}; //1000 0000
	HAL_I2C_Master_Transmit(pca->I2Chandle, PCA9685_I2C_ADDR, txBuf, 2, PCA9685_I2C_TIMEOUT);
	HAL_Delay(10);

	// prescale can only be set when the SLEEP bit of MODE1 reg is set to 1
	/* Put to sleep */
	uint8_t rxBuf;
